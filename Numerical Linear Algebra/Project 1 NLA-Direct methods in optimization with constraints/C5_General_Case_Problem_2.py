...
b_file = 'opt_pr2/b.dad'
C_file = 'opt_pr2/C.dad'
d_file = 'opt_pr2/d.dad'
G_file = 'opt_pr2/G.dad'
g_file = 'opt_pr2/g_.dad'
...
z_list = []
z = np.append(x0,np.append(gamma0,np.append(lambda0,s0)))
...
while True:
    ...
    r_hat = np.append(r_l_hat,np.append(r_g_hat,np.append(r_c_hat,r_s_hat)))
    ...
    if(all(np.absolute(r_l_hat)<epsilon) or all(np.absolute(r_c_hat)<epsilon) or np.absolute(mu)<epsilon or iterations>max_iter):
        ...
        if(iterations>max_iter):
            ...
            break
print('then, we have reached the solution of the problem 2')
